    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="nav-list" aria-label="Main navigation">
          <button className={`nav-item ${activeView === 'workspace' ? 'active' : ''}`} onClick={() => setActiveView('workspace')}><span className="nav-icon">◈</span> Allocation desk</button>
        ...
        <div className="sidebar-bottom"><div className="model-status"><span className="status-dot" /> Pattern engine online</div><p>Upload a few completed weeks, then let Shift Sense prepare the next rota.</p></div>
      </aside>

      <main className="main-content">
        <header className="topbar"><div><p className="eyebrow">Operations / {activeView === 'workspace' ? 'Allocation desk' : 'Learning history'}</p><h1>{activeView === 'workspace' ? 'Build the next rota.' : 'Your learning set.'}</h1></div><div className="topbar-meta"><span className="status-dot" /> API connected <span className="meta-divider" /> 22 Sep 2026</div></header>
        {status.message && <div className={`notice ${status.type}`} role="status">{status.message}</div>}

        {activeView === 'history' ? (
          <section className="history-view"><div className="section-heading"><div><p className="eyebrow">Model context</p><h2>Historical coverage</h2></div><span className="pill">Live from API</span></div><div className="metric-grid"><Metric label="Shifts learned" value={history?.total_historical_shifts ?? '—'} accent="teal" /><Metric label="Weeks represented" value={history?.weeks_of_data ?? '—'} accent="yellow" /><Metric label="AI reasoning" value={history?.ai_reasoning_available ? 'Ready' : 'Fallback'} accent="coral" /></div><div className="history-panel"><div><h3>Week breakdown</h3><p>Recent uploads currently held by the in-memory pattern engine.</p></div>{history?.week_breakdown?.length ? history.week_breakdown.map((week) => <div className="week-row" key={week.week_offset}><span>Week offset {week.week_offset}</span><strong>{week.shift_count} shifts</strong><div className="bar"><i style={{ width: `${Math.min(100, week.shift_count * 5)}%` }} /></div></div>) : <div className="empty-state">No historical workbooks have been uploaded yet.</div>}</div></section>
        ) : (
          <><section className="hero-grid"><div className="hero-copy"><p className="eyebrow">Prediction workspace</p><h2>Turn past patterns into <em>clearer</em> cover.</h2><p>Train on your completed schedules, then generate a proposed week with confidence signals and conflict handling built in.</p></div><div className="hero-note"><span className="note-index">01</span><span>Feed the model your history first</span><span className="arrow">↗</span></div></section>
            <section className="workflow-grid"><article className="workflow-card"><div className="card-number">01 <span>Learning set</span></div><h3>Upload completed weeks</h3><p className="card-help">Add one or more `.xlsx` workbooks with names in the Notes column.</p><FileDrop files={historicalFiles} inputRef={historicalInput} multiple onChange={(event) => setHistoricalFiles(Array.from(event.target.files || []))} /><button className="button secondary" disabled={!historicalFiles.length || busy === 'history'} onClick={uploadHistorical}>{busy === 'history' ? 'Processing...' : 'Add to learning set'} <span>→</span></button></article>
              <article className="workflow-card prediction-card"><div className="card-number">02 <span>New schedule</span></div><h3>Generate allocations</h3><p className="card-help">Choose a blank week and get suggested assignments in seconds.</p><FileDrop files={predictionFile ? [predictionFile] : []} inputRef={predictionInput} onChange={(event) => setPredictionFile(event.target.files?.[0] || null)} /><button className="button primary" disabled={!predictionFile || busy === 'prediction'} onClick={generatePredictions}>{busy === 'prediction' ? 'Analysing...' : 'Generate predictions'} <span>↗</span></button></article></section>
